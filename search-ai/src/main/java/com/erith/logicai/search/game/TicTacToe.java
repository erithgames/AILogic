...
public class TicTacToe extends GameSearch {
    ...
    public Position [] possibleMoves(Position p, boolean player) {
        if (DEBUG) System.out.println("posibleMoves("+p+","+player+")");
        TicTacToePosition pos = (TicTacToePosition)p;
        int count = 0;
        ...
        for (int i=0; i<9; i++) {
            if (pos.board[i] == 0) {
                ...
                ret[count++] = pos2;
                if (DEBUG) System.out.println("    "+pos2);
            }
        }
        return ret;
    }
    public Position makeMove(Position p, boolean player, Move move) {
        if (DEBUG) System.out.println("Entered TicTacToe.makeMove");
        ...
        if (DEBUG) System.out.println("makeMove: m.moveIndex = " + m.moveIndex);
    ...
    public Move createMove() {
        if (DEBUG) System.out.println("Enter blank square index [0,8]:");
        int i = 0;
        try {
